...
class HobbiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create hobby" do
    assert_difference('Hobby.count') do
      post hobbies_url, params: { hobby: { name: @hobby.name, profile_id: @hobby.profile_id, status: @hobby.status } }
    end
  ...
  test "should update hobby" do
    patch hobby_url(@hobby), params: { hobby: { name: @hobby.name, profile_id: @hobby.profile_id, status: @hobby.status } }
